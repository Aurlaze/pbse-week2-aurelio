...
5. **One thing I am unsure about:** I'm not sure how replay should work for transitions to terminal states. Especially, when a client tries again to access a part of a website like POST /v1/bookings/{id}/cancellation on a booking that is already cancelled, I am not completely sure whether the server should respond with 200 OK using the existing record or return 201 Created again. I also don't know if the second response should include the original Location header.